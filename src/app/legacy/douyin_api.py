...
_DOUYIN_AWEME_ID_PATTERN = re.compile(r"/(?:video|note|slides)/(\d+)")


def _aweme_id_from_url(value: str) -> str | None:
    match = _DOUYIN_AWEME_ID_PATTERN.search(value)
    return match.group(1) if match else None


async def _get_douyin_aweme_id(url: str) -> str:
    """Resolve an aweme ID with one redirect request before falling back to f2."""
    from f2.apps.douyin.utils import AwemeIdFetcher

    if aweme_id := _aweme_id_from_url(url):
        return aweme_id
    ...
    try:
        ...
        if aweme_id := _aweme_id_from_url(location):
            logger.info("从短链首次跳转提取 aweme_id，跳过后续页面请求")
            return aweme_id
    except httpx.HTTPError:
        logger.warning("抖音短链快速解析失败，回退 f2 完整解析", exc_info=True)
...
async def _extract_tiktok_api(url: str) -> Optional[dict]:
    ...
    try:
        ...
        proxies = [{"server": proxy_url}] if proxy_url else None
...
async def _extract_douyin_api(url: str, cookie: Optional[str] = None) -> Optional[dict]:
    """
    ...
    try:
        ...
        # 获取 aweme_id
        aweme_id = await _get_douyin_aweme_id(url)
        if not aweme_id:
        ...
        # 构建 kwargs
        douyin_cookie = cookie if cookie is not None else config.get_cookie("douyin")
        kwargs = {
