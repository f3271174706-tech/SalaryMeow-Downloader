...
- 解析、下载、流媒体接口需要邀请码会话。
- 管理记录接口需要 FZP Admin 会话，密码只保存为 scrypt 哈希；未配置哈希时默认禁用。
...
- Admin 会话 Cookie 启用 HttpOnly、SameSite=Strict，生产环境同时启用 Secure。
- Admin 登录与退出校验 Origin，并对失败登录做单进程限流。
...
- 设置强 `DOUYIN_SESSION_SECRET`、邀请码、`FZP_SESSION_SECRET` 和至少 12 位的管理员密码。
