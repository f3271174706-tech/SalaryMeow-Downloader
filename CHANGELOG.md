...
- 新增可配置的 10 分钟元数据 LRU 缓存。
- 新增可配置的邀请码保护开关，默认保持开启。
- 新增共享管理后台跳转和旧版兼容解析记录写入机制。
- 新增受平台、时长、文件大小、并发、条目数和总占用约束的智能预下载。
...
- 新增 systemd 定时清理与应用/Tunnel 简单自愈脚本。
- 新增安全、契约和配置测试。
...
- 固定已验证的顶层依赖版本。
- B站解析会跳过未知动态 MCDN，选择安全白名单内的官方备用线路。
- B站封面统一升级为 HTTPS，并允许加载 Cloudflare Insights 官方脚本。
